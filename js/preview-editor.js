/* ===========================================================
   ...
   • Changes persist in localStorage (this browser only), separately from the live page
   • "Export HTML" downloads a ready-to-publish index.html with
     your text edits applied and uploaded images inlined
   =========================================================== */
(() => {
  "use strict";
  const TKEY = "mc-preview:text:v1";
  const IKEY = "mc-preview:img:v1";
  ...
  bar.innerHTML =
    ...
    '<button data-act="export">⤓ Export draft</button>' +
    '<button data-act="save" class="mc-primary">💾 Save draft</button>' +
  ...
  /* ---- drafts stay in this browser until Codex publishes them ---- */
  function saveToDisk() {
    const status = document.getElementById("mcSaveStatus");
    if (status) {
      status.textContent = "Draft saved in this browser. Tell Codex when ready to publish.";
      status.className = "mc-editbar__status is-ok";
    }
  }

  function exportHTML() {
    const out = JSON.stringify({ text: loadMap(TKEY), img: loadMap(IKEY) }, null, 2);
